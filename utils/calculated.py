...
class calculated:
    ...
    def fighting(self, type: int=0):
        ...
        """
        start_time = time.time()
        attack = cv.imread("./imgs/attack.jpg") if self.platform == _("PC") else cv.imread("./temp/mnq/attack.jpg")
        doubt = cv.imread("./imgs/doubt.jpg") if self.platform == _("PC") else cv.imread("./temp/mnq/doubt.jpg")
        warn = cv.imread("./imgs/warn.jpg") if self.platform == _("PC") else cv.imread("./temp/mnq/warn.jpg")
        tagz = cv.imread("./imgs/tagz.jpg") if self.platform == _("PC") else cv.imread("./temp/mnq/tagz.jpg")
        log.info(_("识别中"))
        while True:
            dt =  time.time() - start_time
            tagz_val = self.scan_screenshot(tagz, pos=(40,0,60,15))["max_val"]
            log.info(f"识别时间{dt} 识别Z {tagz_val}")
            if dt > 10:  # 如果已经识别了10秒还未找到目标图片，则退出循环
            ...
            if attack_result["max_val"] > 0.98:
                ...
                time.sleep(3)
                if  self.platform == _("PC"):
                    target = cv.imread("./imgs/finish_fighting.jpg")  # 識別是否已進入戰鬥，若已進入則跳出迴圈
                else:
                    target = cv.imread("./imgs/finish_fighting.jpg")
                time.sleep(0.3)
                result = self.scan_screenshot(target, pos=(90,100,90,100))
                if result["max_val"] < 0.95:
                    break
            time.sleep(0.1)
        time.sleep(6)
        target = cv.imread("./imgs/auto.jpg") if self.platform == _("PC") else cv.imread("./temp/mnq/auto.jpg")
        start_time = time.time()
